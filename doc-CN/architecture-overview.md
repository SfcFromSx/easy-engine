<!-- MIRROR: docs/architecture/overview.md | SOURCE_SHA256: f5173811ca41 | SYNCED_AT: 2026-03-30T06:07:44Z -->
...
- `query` 接收查询请求，解析 hint 和保留元数据，执行路由，读写缓存，执行只读 SQL，并发送带有显式执行模式元数据的轨迹负载。
- `manager` 消费轨迹负载，将历史持久化，其中包括失败的预编译执行可选的可读参数负载，使用 Calcite 解析 SQL，维护模式统计，并暴露控制面 API 供加速元数据使用。
...
5. `query` 将轨迹负载发布到 Redis，其中包含 `executionMode`，并且对失败的预编译执行包含一个由请求 DTO 参数派生出的可选可读 `parameterPayload`。
6. `manager` 消费该轨迹负载，将其存入 PostgreSQL，通过 `/api/v1/traces` 暴露兼容的轨迹记录，使用 Calcite 解析 SQL，并更新模式统计。
...
- `manager` 必须对当前轨迹负载契约保持向后兼容，包括为较新 query 轨迹新增的可选字段。
