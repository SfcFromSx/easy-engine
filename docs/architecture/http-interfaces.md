...
|---|---|---|---|---|
| `POST` | `/kylin/api/query` | `Implemented` | Execute query requests | Query-only interface; blank or non-query SQL is rejected with the compatibility exception payload, and Kylin-routed prepared requests are literalized inside `query` before execution. |
