...
The foreman updates this document whenever a task resolving a bug or code style issue requires a generalized rule to prevent future regressions.

Before adding a new rule, review the existing list first. If the new lesson overlaps with an existing rule, merge, rewrite, or replace the current wording instead of appending a near-duplicate bullet. Prefer one sharper canonical rule over several partially redundant ones, and note any consolidation in the task log that introduced the change.

## Rules

*(Rules below are curated canonical guidance, not an append-only history log.)*
...
- Keep checked-in test fixtures as explicit inputs. Spring tests that need fixture-backed properties should wire them through visible test configuration such as `@DynamicPropertySource`, not hidden `EnvironmentPostProcessor` or `spring.factories` hooks that silently mutate the `Environment`.
