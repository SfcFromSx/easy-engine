...
service exposes `POST /kylin/api/query` on `8092` and a minimal Trino JDBC
`POST /v1/statement` surface on `8093`.
...
- The profile YAMLs are now MySQL-first across `dev`, `test`, and `pro`; local metadata defaults point at MySQL on `localhost:3307`, and any in-memory H2 usage is limited to test-only fixture sections inside each module's `application-test.yml`.
