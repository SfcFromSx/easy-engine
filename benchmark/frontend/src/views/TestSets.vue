<template>
  <div class="test-sets-view-container">
        ...
        >
          ...
          <template #tip>
            <div class="el-upload__tip">
              {{ $t('testSets.uploadTip') }} 支持可选列：execution_mode、param_json。
            </div>
          </template>
        </el-upload>
      </div>
    ...
    <div class="glass-card table-card">
      <el-table :data="testSets" v-loading="loading" stripe>
        ...
        <el-table-column :label="$t('common.actions')" width="180" fixed="right">
          <template #default="{ row }">
            <el-button size="small" link @click="viewItems(row)">查看</el-button>
    ...
    <el-drawer v-model="drawerVisible" :title="`测试集明细: ${activeSet?.name}`" size="650px" destroy-on-close>
      <div v-for="item in items" :key="item.id" class="item-entry">
        ...
        <div v-if="item.paramJson" class="param-json">
          <strong>Params:</strong> <code>{{ item.paramJson }}</code>
        </div>
      </div>
    </el-drawer>
